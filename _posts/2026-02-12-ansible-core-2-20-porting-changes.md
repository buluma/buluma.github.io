...
In the last six months (Aug 2025–Feb 2026), **ansible-core 2.20** landed with a set of behavioral changes that are easy to miss but can impact real-world automation. The porting guide is the fastest way to spot those changes before they bite you in CI. citeturn5view0

## Changes you’ll actually feel

### PowerShell path handling on Windows
Quote stripping in PowerShell operations has been removed. If you had playbooks that relied on Ansible “fixing” over‑quoted paths, you now need to tighten those paths yourself. This mostly affects Windows copy/fetch workflows. citeturn5view0
...
- Old Paramiko config keys and a few utility/compat shims citeturn5view0

### Deprecation signal you should act on
`INJECT_FACTS_AS_VARS` is now deprecated, which is a strong hint to avoid depending on injected facts as top-level vars and instead use `ansible_facts` directly. citeturn5view0
